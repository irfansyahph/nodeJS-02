...
const _t_agent = require("./t_agent");
const _t_client = require("./t_client");
const _t_policy = require("./t_policy");
...
function initModels(sequelize) {
  ...
  const t_agent = _t_agent(sequelize, DataTypes);
  const t_client = _t_client(sequelize, DataTypes);
  const t_policy = _t_policy(sequelize, DataTypes);
  ...
  t_policy.belongsTo(t_agent, { as: "agent_code_t_agent", foreignKey: "agent_code" });
  t_agent.hasMany(t_policy, { as: "t_policies", foreignKey: "agent_code" });
  t_policy.belongsTo(t_client, { as: "client_number_t_client", foreignKey: "client_number" });
  t_client.hasMany(t_policy, { as: "t_policies", foreignKey: "client_number" });
